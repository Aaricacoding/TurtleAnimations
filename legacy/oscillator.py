...
# ── Constants ──────────────────────────────────────────────────────────────────
NUM_PARTICLES  = 500              # fills the full window
GOLDEN_ANGLE   = 137.508          # degrees — nature's perfect packing angle
# BASE_RADIUS * sqrt(NUM_PARTICLES) + AMPLITUDE ≈ 340 (half of 700px window)
# 13.5 * sqrt(500) ≈ 302 + 38 = 340 ✓
BASE_RADIUS    = 13.5             # scale factor: radius = BASE_RADIUS * sqrt(i)
...
def _particle_color(i, t):
    ...
    # Inner ~60 particles get a white shimmer (scale stays same regardless of N)
    whiteness = max(0.0, 1.0 - i / 80.0) * brightness
...
while True:
    for i, p in enumerate(particles):
        ...
        # Dot size: inner dots larger, outer dots taper — but stay visible
        wave = math.sin(t + i * PHASE_SHIFT)
        base_sz = 0.30 - 0.18 * (i / NUM_PARTICLES)   # 0.30 → 0.12 across range
        sz = max(0.06, base_sz + 0.08 * wave)
        p.shapesize(sz, sz)
